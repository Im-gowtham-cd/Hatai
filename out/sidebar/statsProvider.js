...
class StatsProvider {
    ...
    show() {
        ...
        this.panel = vscode.window.createWebviewPanel('hataiStats', '🛡️ Hatai Dashboard', vscode.ViewColumn.Beside, {
            enableScripts: true,
    ...
    buildHtml() {
        const entries = this.getEntries();
        const config = vscode.workspace.getConfiguration('hatai');
        const allowAIRead = config.get('allowAIReadSecrets', false);
        const allowUserCopy = config.get('allowUserCopySecrets', false);
